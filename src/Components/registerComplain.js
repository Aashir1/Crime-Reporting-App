...
class RegisterComplain extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            name: "",
    ...
    submitCrime = () => {
        ...
        if ( obj.name !== "" && this.emailValidate(obj.email) && obj.description !== "" && obj.time !== "" && obj.description !== "" && obj.contact !== ""){
            ...
            fire.database().ref('/Reporting-App/complains/'+this.state.userUid).push(obj)
                .then(() => {
    ...
    return (
        <div style={{ width: '70%', margin: '0 auto' }}>
            <Paper style={style} zDepth={1}>
                <h1 style={{ textAlign: 'center' }}>Register Complain</h1>
                <TextField
                    value={this.state.name}
                    onChange={(event) => { this.updateState(event, 'name') }}
                    style={styles.textField}
                    hintText=""
                    floatingLabelText="Complained Depart Name"
                /><br />
                <TextField
                    type="email"
                ...
                <div style={{ textAlign: "center" }}>
                    <RaisedButton disabled={this.state.disable} label="Submit" onClick={this.submitCrime} secondary={true} style={{ width: "100%", marginTop: "20px" }} />
                </div>
            </Paper>
